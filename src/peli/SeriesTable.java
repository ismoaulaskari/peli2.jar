...
public class SeriesTable {
    ...
    public void sortMutually(Mutual mutual) {
        ...
        for (j = 0; j <= table.length; j++) {
            ...
            if (debug) {
                System.out.println("Examining " + table[j].getName());
            }

            if (points != lastpoints) { //this player is not part in mutual comparison
                if (debug) {
                    System.out.println("points not lastpoints");
                }
                mutualtreeset.clear();
                ...
                while (tmpplayers.hasMoreElements()) {
                    ...
                    if (debug) {
                        System.out.println("comparing " + names[l - 1] + " with " + lastpoints + "p");
                    }
                }
    ...
    public void print(PrintWriter printwriter) {
        //sortMutually(this.mutual); //does this help for immediate updates
        for (int i = 0; i < size(); i++) {
            printwriter.println(Tools.format(i + 1, 4) + " " + elementAt(i).getRow() + "  ");
        }
    ...
    public String toString() {
        ...
        //sortMutually(this.mutual); //does this help for immediate updates
        for (int i = 0; i < size(); i++) {
            seriestable.append(Tools.format(i + 1, 4) + " " + elementAt(i).getRow() + "  \n");
        }
     ...
     * Print seriestable as html, separate playoff-players and players who didn't make it
     */
    public String toCssHtmlTableWithAnchors(List<Integer> playoffSeparators, String anchorName) {
        StringBuffer seriestable = new StringBuffer();
        seriestable.append("<table class=\"seriestable\">").append(System.getProperty("line.separator"));
        ...
        for (int i = 0; i < size(); i++) {
            ...
            if (i == 0) {
                seriestable.append("<tr class=\"seriestablerow\">").append(System.getProperty("line.separator")).
                        append("<td class=\"seriesorder\"><a name=\"").append(anchorName).append("\"/>").append(i + 1).append("</td>").append(elementAt(i).getCssHtmlTableRow()).
                        append(System.getProperty("line.separator")).append("</tr>").append(System.getProperty("line.separator"));
            } else {
                seriestable.append("<tr class=\"seriestablerow\">").append(System.getProperty("line.separator")).
                        append("<td class=\"seriesorder\">").append(i + 1).append("</td>").append(elementAt(i).getCssHtmlTableRow()).
